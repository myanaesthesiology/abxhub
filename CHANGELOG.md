...
- Completed comprehensive source reconciliation and structural data-quality audit without adding new clinical modules.
- Canonicalised 224 bundled-PDF page links and validated 1,673 source-reference objects.
- Removed unavailable institution-specific antibiogram/TDM/CRRT/product layers from the v1.0 core release gate.
- Retired user-facing institutional “not configured” states.
- Added `scripts/validate_release.py` and `scripts/smoke_static.py`; GitHub Actions now runs both plus `node --check app.js` before deployment.
- Added source reconciliation, data-quality audit and final pre-v1.0 audit documents.
- Updated service-worker/app-shell cache to v0.20.0.
