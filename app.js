...
function recordRoute(route){
  ...
  if(state.restoringHistory) return false;
  const st=history.state;
  if(st?.abxRoute && sameRoute(st.abxRoute,route)) return false;
  ...
  // Forward/new navigation always starts at the top. Back/forward restoration is handled separately by restoreRoute().
...
function sourcePageHasTables(sourceId,pdfPage){ return !!((sourcePageParity(sourceId,pdfPage)?.tables||[]).length); }
function sourceTranscriptHTML(text=''){
  const normalized=String(text||'').replace(/\r/g,'').replace(/\u00ad/g,'').replace(/\u00a0/g,' ').replace(/\s+[•●▪◦‣]\s+/g,'\n• ').replace(/\s+»\s+/g,'\n• ');
  const lines=normalized.split('\n').map(x=>x.replace(/[ \t]+/g,' ').trim());
  const blocks=[]; let para=[], listType='', listItems=[];
  const tidy=s=>s.replace(/-\s+([a-zα-ω])/gi,'-$1').replace(/\s+([,.;:!?])/g,'$1').replace(/\(\s+/g,'(').replace(/\s+\)/g,')').replace(/\s{2,}/g,' ').trim();
  const flushPara=()=>{if(para.length){blocks.push(`<p>${esc(tidy(para.join(' ')))}</p>`);para=[];}};
  const flushList=()=>{if(listItems.length){const tag=listType==='ol'?'ol':'ul';blocks.push(`<${tag}>${listItems.map(x=>`<li>${esc(tidy(x))}</li>`).join('\n')}</${tag}>`);listItems=[];listType='';}};
  const isHeading=line=>{
    if(!line || line.length>96 || /[.!?]$/.test(line)) return false;
    const letters=(line.match(/[A-Za-z]/g)||[]).length, upper=(line.match(/[A-Z]/g)||[]).length;
    if(letters>=4 && upper/letters>.82) return true;
    return /^(chapter|appendix|section|table|figure|foreword|disclaimer|references?|antimicrobial stewardship|drug|dose and route|duration|monitoring|general principles?|treatment|management|prevention|introduction|conclusion|recommendations?)\b/i.test(line) && line.length<72;
  };
  for(let i=0;i<lines.length;i++){
    const line=lines[i];
    if(!line){flushPara();flushList();continue;}
    const bullet=line.match(/^[•●▪◦‣–-]\s+(.*)$/); const numbered=line.match(/^\d+[.)]\s+(.*)$/);
    if(bullet||numbered){flushPara(); const type=numbered?'ol':'ul'; if(listItems.length&&listType!==type)flushList(); listType=type; listItems.push((bullet||numbered)[1]); continue;}
    if(listItems.length){
      if(isHeading(line)){flushList();blocks.push(`<div class="source-transcript-heading">${esc(line)}</div>`);continue;}
      listItems[listItems.length-1]+=' '+line; continue;
    }
    if(isHeading(line)){flushPara();blocks.push(`<div class="source-transcript-heading">${esc(line)}</div>`);continue;}
    para.push(line);
  }
  flushPara();flushList();
  return blocks.join('\n') || '<p class="muted">No searchable transcript text on this page.</p>';
}
function sourceTranscriptDetailsHTML(sourceId,pdfPage,text=''){
  const open=sourcePageHasTables(sourceId,pdfPage)?'open':'';
  return `<details class="raw-source-text" ${open}><summary><b>Searchable source transcript</b><span class="muted tiny">Reflowed for reading</span></summary><div class="source-transcript">${sourceTranscriptHTML(text)}</div></details>`;
}
...
function hpusmSourcePagesHTML(pages=[]){
  ...
  return `<div class="section-head"><div><h3>Exact HPUSM source pages</h3><p>Responsive HTML tables / WebP figures where available, plus searchable transcript and exact PDF</p></div></div><div>${wanted.map(p=>`<details class="inset-card hpusm-source-page source-page-card"><summary><b>HPUSM p${esc(p.page)}</b><span class="muted small">${esc(p.section||'Source page')}</span></summary>${sourcePageEnhancementsHTML('hpusm',p.pdfPage,{open:true})}${sourceTranscriptDetailsHTML('hpusm',p.pdfPage,p.text||'')}<button class="link-btn" data-source-ref='${encodeURIComponent(JSON.stringify({source:'hpusm',page:p.page,pdfPage:p.pdfPage}))}'>Open exact bundled PDF page →</button></details>`).join('')}</div>`;
}
...
function bindGlobal(){
  ...
  document.addEventListener('keydown',e=>{
    ...
    if(e.key==='Enter'&&e.target.id==='universalSearch'){e.preventDefault();const q=e.target.value.trim();e.target.blur();if(q)renderSearchResults(q)}
  });
  document.addEventListener('click',e=>{if(e.target.id==='searchGo'){const inp=$('#universalSearch'),q=inp?.value.trim();inp?.blur();if(q)renderSearchResults(q)}});
}
...
function switchPrimaryNav(tab){
  // A tab selection is forward navigation: show the destination from its top.
  // The page being left is still captured in history by recordRoute(), so browser/app Back restores its exact prior scroll position.
...
function msicSectionCard(sec,kind,index){
  ...
  return `<div class="source-meta"><span class="pill icu">${kind==='chapter'?'Chapter '+esc(sec.number):'Appendix '+esc(sec.letter||String.fromCharCode(65+index))}</span><span class="pill">Book p${esc(sec.bookPages||'')}</span><span class="pill">PDF p${esc(sec.pdfPages||'')}</span></div><h3>${esc(sec.title||'MSIC section')}</h3>${links?`<div class="list" style="margin:10px 0">${links}</div>`:''}<div class="notice"><b>Bundled-source parity:</b> Tables are rendered as responsive HTML when detected from the bundled source. Searchable transcript and the exact PDF remain available for surrounding text and verification.</div>${(sec.pages||[]).map(p=>`<details class="inset-card msic-source-page source-page-card"><summary><b>MSIC book p${esc(p.bookPage)}</b><span class="muted small">PDF p${esc(p.pdfPage)}</span></summary>${sourcePageEnhancementsHTML('msic',p.pdfPage,{open:true})}${sourceTranscriptDetailsHTML('msic',p.pdfPage,p.text||'')}<button class="link-btn" data-source-ref='${encodeURIComponent(JSON.stringify({source:'msic',page:p.bookPage,pdfPage:p.pdfPage}))}'>Open exact bundled PDF page →</button></details>`).join('')}`;
}
...
function bindGlobal(){
  ...
  document.addEventListener('keydown',e=>{if(e.key==='Escape')closeModal();if(e.key==='Enter'&&e.target.id==='universalSearch'){e.preventDefault();const q=e.target.value.trim();e.target.blur();if(q)renderSearchResults(q)}if(e.key==='Enter'&&e.target.id==='searchPageInput'){e.preventDefault();const q=e.target.value.trim();e.target.blur();if(q){updateSearchRoute(q);$('#searchPageResults').innerHTML=searchResultsHTML(q)}}});
}
/* ==================== end v0.20.0 overrides ==================== */



/* ==================== v0.20.0-r21 UX refinements ==================== */
state.searchTypeFilter='all';

function r21SearchTypeLabel(type){
  return ({drug:'Drugs',condition:'Diseases',organism:'Organisms',resistance:'Resistance',tool:'Tools',naglink:'Tools'})[type]||type;
}
function r21SearchFilterRow(){
  const opts=[['all','All'],['drug','Drugs'],['condition','Diseases'],['organism','Organisms'],['resistance','Resistance'],['tool','Tools']];
  return `<div class="search-type-filters" role="group" aria-label="Filter search results">${opts.map(([id,label])=>`<button class="search-filter-btn ${state.searchTypeFilter===id?'active':''}" data-search-filter="${id}">${label}</button>`).join('')}</div>`;
}
const r20SearchResultsHTML=searchResultsHTML;
searchResultsHTML=function(q){
  const all=search(q); const filter=state.searchTypeFilter||'all';
  const hits=filter==='all'?all:all.filter(h=>filter==='tool'?(h.type==='tool'||h.type==='naglink'):h.type===filter);
  const syn=parseSynthesis(q);
  return `${filter==='all'&&syn.tokens.length>=2?renderSynthesisPreview(syn):''}<div class="search-filter-summary"><span>${hits.length} result${hits.length===1?'':'s'}</span>${filter!=='all'?`<span class="pill">${esc(r21SearchTypeLabel(filter))}</span>`:''}</div>${hits.length?`<div class="search-results">${hits.map(searchHitHTML).join('')}</div>`:`<div class="search-results"><div class="empty"><b>No ${filter==='all'?'structured':esc(r21SearchTypeLabel(filter).toLowerCase())} match</b>Try another result type or a different keyword.</div></div>`}`;
};
renderSearchResults=function(q){
  q=String(q||'').trim(); if(!q)return renderHome();
  state.searchTypeFilter='all';
  recordRoute({view:'search',q}); state.nav='home';navActive();
  $('#main').innerHTML=`<div class="search-page-head"><div class="entity-title"><button class="back-btn" data-back aria-label="Back">‹</button><div class="grow"><h1>Search</h1><p>Structured AbxHub results</p></div></div><div class="search-wrap"><input id="searchPageInput" class="search" autocomplete="off" spellcheck="false" value="${esc(q)}" placeholder="Search AbxHub…"><button id="searchPageGo" class="search-btn" aria-label="Search">⌕</button></div>${r21SearchFilterRow()}</div><div id="searchPageResults">${searchResultsHTML(q)}</div>${footer()}`;
};

function r21DiseaseCategory(x){
  const s=slug(`${x.category||''} ${x.name||''}`);
  if(/paediatric|pediatric|neonatal/.test(s)) return 'Paediatric';
  if(/respiratory|pneum|pulmon|thorac/.test(s)) return 'Respiratory';
  if(/cns|central nervous|mening|encephal|neuro|spine/.test(s)) return 'CNS';
  if(/urinary|uro|genitourinary|pyelo|cystitis/.test(s)) return 'Urinary';
  if(/obstetric|gynaec|gynec|pregnan|sti|sexually|pelvic inflammatory/.test(s)) return 'O&G / STI';
  if(/gastro|intra abdominal|hepat|biliar|gi |oral|dental/.test(s)) return 'GI / Dental';
  if(/skin|soft tissue|ssti|orthop|bone|joint|wound/.test(s)) return 'SSTI / Bone';
  if(/ent|ocular|ophthalm|ear|sinus|pharyng|tonsil/.test(s)) return 'ENT / Ocular';
  if(/tropical|paras|fung|hiv|opportun|mycobact|zoono/.test(s)) return 'Tropical / Opportunistic';
  return 'Other';
}
function r21DrugCategory(x){
  const s=slug(`${x.class||''} ${x.name||''}`);
  if(/antifungal|azole|echinocandin|amphotericin|polyene/.test(s)) return 'Antifungal';
  if(/antiviral|virus|acyclovir|ganciclovir|oseltamivir/.test(s)) return 'Antiviral';
  if(/antimalarial|antiparas|anthelm|protozo|scab|ivermectin/.test(s)) return 'Antiparasitic';
  if(/mycobact|antituberc|lepro/.test(s)) return 'Antimycobacterial';
  return 'Antibacterial';
}
function r21OrganismCategory(x){
  if(x._type==='resistance') return 'Resistance';
  const s=slug(`${x.gram||''} ${x.name||''}`);
  if(/virus|coronavirus|herpes|orthomyxo|polyoma|pox/.test(s)) return 'Viruses';
  if(/fung|yeast|mould|mold|dermatophyte|microspor/.test(s)) return 'Fungi';
  if(/paras|protozo|amoeba|ectoparas|mite/.test(s)) return 'Parasites';
  return 'Bacteria';
}
function r21DirectoryCategory(x,kind){return kind==='condition'?r21DiseaseCategory(x):kind==='drug'?r21DrugCategory(x):r21OrganismCategory(x)}
function r21DirectoryRows(arr,kind){
  return arr.map(x=>{const t=kind==='mixed'?(x._type||'organism'):kind;const letter=(x.name||'#').trim().charAt(0).toUpperCase();return `<button class="list-row directory-row" data-dir-letter="${/[A-Z]/.test(letter)?letter:'#'}" data-open="${t}" data-id="${x.id}"><div class="grow"><strong>${esc(x.name)}</strong><small>${esc(x.class||x.category||x.gram||(t==='resistance'?'Resistance / phenotype':''))}</small></div><span class="chev">›</span></button>`}).join('')||'<div class="empty"><b>No matches</b></div>';
}
renderDirectory=function(kind){
  recordRoute({view:'directory',kind}); state.nav=kind==='condition'?'diseases':kind==='drug'?'drugs':'organisms';navActive();
  let arr,title,sub;
  if(kind==='drug'){arr=state.db.drugs.map(x=>({...x}));title='Drugs';sub='Antimicrobial monographs, dosing and linked indications';}
  else if(kind==='condition'){arr=state.db.conditions.map(x=>({...x}));title='Diseases & syndromes';sub='Source-specific empirical and targeted pathways';}
  else {arr=[...state.db.organisms.map(x=>({...x,_type:'organism'})),...state.db.resistance.map(x=>({...x,_type:'resistance'}))];title='Organisms & resistance';sub='Organism profiles, phenotypes and MDR pathways';}
  arr.sort((a,b)=>String(a.name||'').localeCompare(String(b.name||'')));
  const cats=uniq(arr.map(x=>r21DirectoryCategory(x,kind))).filter(x=>x!=='Other'); if(arr.some(x=>r21DirectoryCategory(x,kind)==='Other'))cats.push('Other');
  const letters=uniq(arr.map(x=>((x.name||'#').trim().charAt(0).toUpperCase()))).filter(x=>/[A-Z]/.test(x)).sort();
  $('#main').innerHTML=`${pageHeader(title,sub)}<div class="directory-controls"><div class="field"><input id="dirFilter" autocomplete="off" placeholder="Filter ${esc(title.toLowerCase())}…"></div><div class="directory-categories"><button class="dir-cat active" data-dir-cat="All">All</button>${cats.map(c=>`<button class="dir-cat" data-dir-cat="${esc(c)}">${esc(c)}</button>`).join('')}</div><div class="alphabet-jump" aria-label="Alphabet jump">${letters.map(l=>`<button data-dir-alpha="${l}">${l}</button>`).join('')}</div></div><div id="dirCount" class="tiny muted directory-count"></div><div id="dirList" class="list">${r21DirectoryRows(arr,kind)}</div>${footer()}`;
  let category='All';
  const draw=()=>{const q=slug($('#dirFilter').value);const filtered=arr.filter(x=>(category==='All'||r21DirectoryCategory(x,kind)===category)&&(!q||slug([x.name,...(x.aliases||[]),x.class,x.category,x.gram].join(' ')).includes(q)));$('#dirList').innerHTML=r21DirectoryRows(filtered,kind);$('#dirCount').textContent=`${filtered.length} of ${arr.length}`;};
  $('#dirFilter').addEventListener('input',draw);
  $$('.dir-cat').forEach(b=>b.addEventListener('click',()=>{category=b.dataset.dirCat;$$('.dir-cat').forEach(x=>x.classList.toggle('active',x===b));draw();}));
  $$('.alphabet-jump button').forEach(b=>b.addEventListener('click',()=>{$(`[data-dir-letter="${b.dataset.dirAlpha}"]`)?.scrollIntoView({behavior:'smooth',block:'start'});}));
  draw();
};

renderTools=function(){
  recordRoute({view:'tools'});state.nav='tools';navActive();
  const section=(title,sub,cards)=>`<section class="tool-section"><div class="section-head"><div><h2>${esc(title)}</h2><p>${esc(sub)}</p></div></div><div class="tool-grid">${cards}</div></section>`;
  $('#main').innerHTML=`${pageHeader('Tools','Offline clinical utilities, stewardship workflows and bundled guideline readers')}
  ${section('Dosing & monitoring','Patient-specific calculations and ICU dose optimisation',`${toolCard('∑','Patient dose','Cockcroft–Gault, BMI, IBW/AdjBW + drug context','dose')}${toolCard('↻','Renal / RRT','Structured ICU dosing + NAG cross-check','renal')}${toolCard('◎','TDM','Guided NAG + MSIC monitoring workflow','tdm')}${toolCard('⏱','Prolonged infusion','Loading, maintenance, preparation + drug links','infusion')}`)}
  ${section('Stewardship & decision support','Selection, comparison, de-escalation and allergy/prophylaxis support',`${toolCard('≈','Spectrum explorer','Expected activity + Wellington source overlay','spectrum')}${toolCard('≡','Compare antibiotics','Up to five agents side-by-side','compare')}${toolCard('⇄','IV → PO','Interactive NAG criteria + conversion table','ivpo')}${toolCard('A?','Antibiotic allergy','PEN-FAST + NAG Appendix 3 pathway','allergy')}${toolCard('✚','Antimicrobial prophylaxis','Adult + paediatric NAG chemoprophylaxis','prophylaxis')}`)}
  ${section('Special populations & pathways','Pregnancy/lactation and primary-care algorithms',`${toolCard('♀','Pregnancy / lactation','All 83 bundled NAG Appendix 4 rows','pregnancy')}${toolCard('PC','Primary care pathways','Nine structured NAG Section C pathways','primarycare')}`)}
  ${section('Guidelines & sources','Full bundled-source readers and provenance',`${toolCard('LOCAL','HPUSM ASP 2025','11 chapters · full 25-page source reader','hpusm')}${toolCard('ICU','MSIC Adult ICU 2023','21 chapters + 7 appendices structured','msic')}${toolCard('MDR','MSIDC MDR Gram-Negatives 2024','6 treatment sections + IPC + 5 appendices','msidc')}${toolCard('WELL','Wellington ICU Spectrum 2022','17 organism columns · 30 source rows','wellington')}${toolCard('NAG','NAG structured','Offline recommendations + official update check','nag')}${toolCard('ⓘ','Sources & app','Guideline hierarchy, freshness and scope','sources')}`)}${footer()}`;
};

const r20RenderDrug=renderDrug;
function r21SetupDrugNav(){
  const nav=$('.drug-subnav'); if(!nav)return;
  if(state._drugSpyHandler)window.removeEventListener('scroll',state._drugSpyHandler);
  const buttons=$$('[data-drug-section]',nav);
  const sections=buttons.map(b=>({b,el:$(b.dataset.drugSection)})).filter(x=>x.el);
  const setActive=()=>{const y=window.scrollY+125;let current=sections[0];if(window.scrollY+window.innerHeight>=document.documentElement.scrollHeight-36)current=sections[sections.length-1];else for(const x of sections){if(x.el.offsetTop<=y)current=x;else break;}buttons.forEach(b=>b.classList.toggle('active',current&&b===current.b));};
  state._drugSpyHandler=setActive;window.addEventListener('scroll',setActive,{passive:true});setActive();
}
function r21CollapseOccurrences(){
  $$('.regimen-occurrences .source-group').forEach((g,gi)=>{const rows=$$('.source-group-item',g);if(rows.length<=3)return;rows.slice(3).forEach(x=>x.classList.add('occ-hidden'));const host=$('.source-group-items',g);host.insertAdjacentHTML('afterend',`<button class="occ-toggle" data-occ-group="${gi}" aria-expanded="false">Show ${rows.length-3} more</button>`);});
}
renderDrug=function(id){
  r20RenderDrug(id);
  const main=$('#main'),quick=$('.quick-dose',main),tabs=$('.tabs',main);if(!main||!tabs)return;
  quick?.setAttribute('id','drugOverview');tabs.classList.add('drug-subnav');
  const labels=['Overview','Dosing','PK/PD','Sources']; const targets=['#drugOverview','#doseSec','#pkpdSec','#sourceSec'];
  labels.forEach((lab,i)=>{const b=$$('.tab-btn',tabs).find(x=>x.textContent.trim()===lab);if(b){b.dataset.jump=targets[i];b.dataset.drugSection=targets[i];b.classList.toggle('active',i===0);}});
  const renal=$$('.tab-btn',tabs).find(x=>/^Renal\/RRT/.test(x.textContent.trim()));if(renal){renal.textContent='Renal/RRT ↗';renal.classList.add('external-tab');}
  r21CollapseOccurrences();r21SetupDrugNav();
};

const r20RenderCondition=renderCondition;
renderCondition=function(id){
  r20RenderCondition(id);
  const main=$('#main');if(!main)return;
  const cards=$$('.card',main),assessment=cards.find(c=>c.querySelector(':scope > p')&&c.querySelector(':scope > p')?.textContent.trim()); if(assessment)assessment.id='conditionAssessment';
  const micro=cards.find(c=>/microbiology/i.test(c.querySelector('h3')?.textContent||''));if(micro)micro.id='conditionMicrobiology';
  const regimenHeading=$$('h4',main).find(x=>/Source-specific regimen map/i.test(x.textContent||''));const treatment=regimenHeading?.closest('section')||$$('.section-head',main).find(x=>/Empirical \/ targeted/i.test(x.textContent||''));if(treatment)treatment.id='conditionTreatment';
  const reviewHeading=$$('h4',main).find(x=>/48.?72 h review/i.test(x.textContent||''));const review=reviewHeading?.closest('section')||reviewHeading?.closest('.card');if(review)review.id='conditionReview';
  const sources=cards.find(c=>/^Source provenance/i.test(c.querySelector('h3')?.textContent||''));if(sources)sources.id='conditionSources';
  if(!assessment)return;
  const items=[['Assessment','#conditionAssessment'],['Empiric treatment','#conditionTreatment'],['Microbiology','#conditionMicrobiology'],['48–72 h review','#conditionReview'],['Sources','#conditionSources']].filter(([,sel])=>$(sel));
  const bar=document.createElement('div');bar.className='condition-jumpbar';bar.innerHTML=items.map(([lab,sel])=>`<button data-jump="${sel}">${lab}</button>`).join('');assessment.insertAdjacentElement('afterend',bar);
};

const r20SourceParityTableHTML=sourceParityTableHTML;
sourceParityTableHTML=function(t,index=0){
  let html=r20SourceParityTableHTML(t,index);if(!localStorage.getItem('abxhub-table-swipe-hint-seen'))html=html.replace('<div class="source-table-wrap">','<div class="table-swipe-hint">Swipe ↔ to view all columns</div><div class="source-table-wrap">');return html;
};

function r21CompactSourceStatus(toolId){
  const counts={hpusm:2,msic:2,msidc:1,nag:2};const count=counts[toolId]||0;if(!count)return;
  const main=$('#main');if(!main)return;const notices=[...main.children].filter(x=>x.classList?.contains('notice')).slice(0,count);if(!notices.length)return;
  const key=`abxhub-source-status-seen-${toolId}`;const panel=document.createElement('details');panel.className='card source-status-panel';if(!localStorage.getItem(key))panel.open=true;
  panel.innerHTML=`<summary><span><b>ⓘ Source status</b><small>Provenance, scope and reconciliation notes</small></span><span class="chev">＋</span></summary><div class="source-status-body"></div>`;const body=$('.source-status-body',panel);notices.forEach(n=>body.appendChild(n));
  const header=[...main.children].find(x=>x.classList?.contains('entity-title'));if(toolId==='msidc'){const prominent=[...main.children].find(x=>x.classList?.contains('notice'));if(prominent)prominent.insertAdjacentElement('afterend',panel);else if(header)header.insertAdjacentElement('afterend',panel);else main.prepend(panel);}else if(header)header.insertAdjacentElement('afterend',panel);else main.prepend(panel);localStorage.setItem(key,'1');
}
function r21EnhanceSourceReader(toolId){
  const main=$('#main');if(!main)return;
  const ids={hpusm:['hpusmChapter','hpusmSearch'],msic:['msicSection','msicSearch'],msidc:['msidcSection','msidcSearch']}[toolId];
  if(ids){const first=$(`#${ids[0]}`);first?.closest('.form-grid')?.classList.add('source-reader-controls');}
  r21CompactSourceStatus(toolId);
}
const r20RenderTool=renderTool;
renderTool=function(id,opts={}){r20RenderTool(id,opts);requestAnimationFrame(()=>r21EnhanceSourceReader(id));};

/* r21 delegated micro-interactions */
document.addEventListener('click',e=>{
  const occ=e.target.closest('.occ-toggle');if(occ){const g=occ.closest('.source-group'),hidden=$$('.source-group-item.occ-hidden',g),expanded=occ.getAttribute('aria-expanded')==='true';if(expanded){$$('.source-group-item',g).slice(3).forEach(x=>x.classList.add('occ-hidden'));occ.textContent=`Show ${Math.max(0,$$('.source-group-item',g).length-3)} more`;occ.setAttribute('aria-expanded','false');}else{hidden.forEach(x=>x.classList.remove('occ-hidden'));occ.textContent='Show less';occ.setAttribute('aria-expanded','true');}return;}
  const sf=e.target.closest('.search-filter-btn');if(sf){state.searchTypeFilter=sf.dataset.searchFilter||'all';$$('.search-filter-btn').forEach(b=>b.classList.toggle('active',b===sf));const q=$('#searchPageInput')?.value||'';const out=$('#searchPageResults');if(out)out.innerHTML=searchResultsHTML(q);return;}
  if(e.target.closest('.source-table-wrap')){localStorage.setItem('abxhub-table-swipe-hint-seen','1');$$('.table-swipe-hint').forEach(x=>x.remove());}
  if(e.target.closest('#installBtn')){localStorage.setItem('abxhub-install-hint-seen','1');$('#installBtn')?.classList.remove('show-install-hint');}
});
window.addEventListener('beforeinstallprompt',()=>{requestAnimationFrame(()=>{const b=$('#installBtn');if(b&&!localStorage.getItem('abxhub-install-hint-seen')){b.classList.add('show-install-hint');setTimeout(()=>b.classList.remove('show-install-hint'),5500);}});});
/* ==================== end v0.20.0-r21 UX refinements ==================== */


/* ==================== v0.20.0-r22 organism source-detail expansion ==================== */
function r22List(items=[]){return items?.length?`<ul class="small">${items.map(x=>`<li>${esc(x)}</li>`).join('')}</ul>`:'<div class="small muted">None listed in this source row.</div>';}
function r22RelevantConditions(o,sourceId){
  const linked=new Set(o.links||[]);
  return (state.db.conditions||[]).map(c=>({c,recs:(c.recommendations||[]).filter(r=>r.source===sourceId)})).filter(x=>x.recs.length&&((x.c.organisms||[]).includes(o.id)||linked.has(x.c.id)));
}
function r22SourceRecommendationRow(r,{showRisk=true}={}){
  return `<div class="organism-source-regimen">${showRisk?`<div class="source-field"><b>Risk / source stratum</b><span>${esc(r.label||'Not separately stratified')}</span></div>`:''}<div class="source-field"><b>Preferred</b>${r22List(r.preferred||[])}</div><div class="source-field"><b>Alternative</b>${r22List(r.alternative||[])}</div>${r.duration?`<div class="source-field"><b>Duration / review</b><span>${esc(r.duration)}</span></div>`:''}${(r.notes||[]).length?`<div class="source-field"><b>Comment</b>${r22List(r.notes)}</div>`:''}<div class="tag-row"><button class="pill" data-source-ref='${encodeURIComponent(JSON.stringify({source:r.source,page:r.page,pdfPage:r.pdfPage,url:r.url,pdfUrl:r.pdfUrl}))}'>${refText({source:r.source,page:r.page,pdfPage:r.pdfPage})}</button></div></div>`;
}
function r22NagQuickGuideHTML(o){
  const rows=state.db.organismSourceDetail?.nagQuickGuide?.[o.id]||[];
  if(!rows.length)return '';
  return `<details class="card organism-source-detail nag-detail" open><summary><span><b>NAG organism antimicrobial quick guide</b><small>Preferred · Alternative · Comment</small></span>${sourcePill('nag')}</summary><div class="organism-source-body">${rows.map((r,i)=>`<details class="inset-card" ${rows.length===1||i===0?'open':''}><summary><b>${esc(r.label)}</b><span class="chev">＋</span></summary><div class="source-field"><b>Preferred</b>${r22List(r.preferred||[])}</div><div class="source-field"><b>Alternative</b>${r22List(r.alternative||[])}</div><div class="source-field"><b>Comment</b>${r22List(r.comments||[])}</div><div class="btn-row"><a class="secondary-btn" href="${esc(r.url)}" target="_blank" rel="noopener">Check current NAG row ↗</a></div></details>`).join('')}<div class="notice warn"><b>Interpretation guard:</b> This organism-level quick guide does not replace infection-site, severity, phenotype/MIC or isolate-specific susceptibility assessment.</div></div></details>`;
}
function r22NagSyndromeHTML(o){
  const rows=r22RelevantConditions(o,'nag');
  if(!rows.length)return '';
  return `<details class="card organism-source-detail nag-detail"><summary><span><b>NAG syndrome-specific recommendations</b><small>${rows.length} linked diagnosis${rows.length===1?'':'es'} · all structured Preferred / Alternative / Comment-duration rows</small></span>${sourcePill('nag')}</summary><div class="organism-source-body"><div class="notice">These are NAG disease/syndrome rows in which this organism is explicitly linked in AbxHub. They remain separate from the organism quick guide above.</div>${rows.map(({c,recs})=>`<details class="organism-diagnosis"><summary><span><b>${esc(c.name)}</b><small>${recs.length} NAG source row${recs.length===1?'':'s'}</small></span><span class="chev">＋</span></summary>${(c.riskFactors||[]).length?`<div class="source-field"><b>Risk stratification / modifiers</b><div class="tag-row">${c.riskFactors.map(x=>`<span class="pill">${esc(x)}</span>`).join('')}</div></div>`:''}${recs.map(r=>r22SourceRecommendationRow(r)).join('')}<div class="btn-row"><button class="secondary-btn" data-open="condition" data-id="${esc(c.id)}">Open full ${esc(c.name)} pathway</button></div></details>`).join('')}</div></details>`;
}
function r22HpusmHTML(o){
  const rows=r22RelevantConditions(o,'hpusm');
  const linkedRes=(o.links||[]).map(id=>state.db.resistance.find(r=>r.id===id)).filter(Boolean);
  const resCards=linkedRes.flatMap(r=>(r.sourceCards||[]).filter(sc=>sc.source==='hpusm').map(sc=>({r,sc})));
  if(!rows.length&&!resCards.length)return '';
  const pages=uniq([
    ...rows.flatMap(x=>x.recs.map(r=>Number(r.pdfPage||r.page)).filter(Number.isFinite)),
    ...resCards.map(x=>Number(x.sc.pdfPage||x.sc.page)).filter(Number.isFinite)
  ]);
  return `<details class="card organism-source-detail local-detail" open><summary><span><b>HPUSM ASP 2025 — diagnosis / phenotype-specific therapy</b><small>${rows.length} mapped ${rows.length===1?'diagnosis':'diagnoses'}${resCards.length?` · ${resCards.length} resistance/phenotype source card${resCards.length===1?'':'s'}`:''}</small></span>${sourcePill('hpusm')}</summary><div class="organism-source-body">${rows.map(({c,recs},i)=>`<details class="organism-diagnosis" ${rows.length<=2||i===0?'open':''}><summary><span><b>${esc(c.name)}</b><small>${recs.length} local source row${recs.length===1?'':'s'}</small></span><span class="chev">＋</span></summary><div class="source-field"><b>Diagnosis</b><span>${esc(c.name)}</span></div>${(c.riskFactors||[]).length?`<div class="source-field"><b>Risk stratification / modifiers</b><div class="tag-row">${c.riskFactors.map(x=>`<span class="pill">${esc(x)}</span>`).join('')}</div></div>`:''}${recs.map(r=>`<div class="organism-source-regimen"><div class="source-field"><b>Risk / source stratum</b><span>${esc(r.label||'Not separately stratified')}</span></div><div class="source-field"><b>Recommended antimicrobial therapy — preferred</b>${r22List(r.preferred||[])}</div><div class="source-field"><b>Recommended antimicrobial therapy — alternative</b>${r22List(r.alternative||[])}</div><div class="source-field"><b>Duration (days / source wording)</b><span>${esc(r.duration||'Not separately stated in this structured row')}</span></div>${(r.notes||[]).length?`<div class="source-field"><b>Comment</b>${r22List(r.notes)}</div>`:''}<div class="tag-row"><button class="pill local" data-source-ref='${encodeURIComponent(JSON.stringify({source:'hpusm',page:r.page,pdfPage:r.pdfPage}))}'>${refText({source:'hpusm',page:r.page,pdfPage:r.pdfPage})}</button></div></div>`).join('')}<div class="btn-row"><button class="secondary-btn" data-open="condition" data-id="${esc(c.id)}">Open full diagnosis pathway</button></div></details>`).join('')}${resCards.length?`<details class="organism-diagnosis" ${rows.length?'':'open'}><summary><span><b>Targeted resistance / phenotype therapy</b><small>${resCards.length} HPUSM source card${resCards.length===1?'':'s'}</small></span><span class="chev">＋</span></summary>${resCards.map(({r,sc})=>`<div class="organism-source-regimen"><div class="source-field"><b>Diagnosis / phenotype</b><span>${esc(r.name)}</span></div><div class="source-field"><b>Risk stratification / source context</b><span>${esc(sc.title||'Targeted HPUSM pathway')}</span></div><div class="source-field"><b>Recommended antimicrobial therapy / source detail</b><span>${esc(sc.text||'')}</span>${(sc.bullets||[]).length?r22List(sc.bullets):''}</div><div class="tag-row"><button class="pill local" data-source-ref='${encodeURIComponent(JSON.stringify({source:'hpusm',page:sc.page,pdfPage:sc.pdfPage}))}'>${refText({source:'hpusm',page:sc.page,pdfPage:sc.pdfPage})}</button></div></div>`).join('')}</details>`:''}${pages.length?`<details class="inset-card full-source-pages"><summary><b>Full HPUSM source pages</b><span class="pill">${pages.length} page${pages.length===1?'':'s'}</span></summary>${hpusmSourcePagesHTML(pages)}</details>`:''}</div></details>`;
}
function r22FindSourcePage(sourceId,pdfPage){
  const p=Number(pdfPage);
  if(sourceId==='hpusm')return (state.db.hpusmGuide?.sourcePages||[]).find(x=>Number(x.pdfPage||x.page)===p)||null;
  if(sourceId==='msidc')return [...(state.db.msidcGuide?.sections||[]),...(state.db.msidcGuide?.appendices||[])].flatMap(x=>x.pages||[]).find(x=>Number(x.pdfPage)===p)||null;
  if(sourceId==='msic')return [...(state.db.msicGuide?.sections||[]),...(state.db.msicGuide?.appendices||[])].flatMap(x=>x.pages||[]).find(x=>Number(x.pdfPage)===p)||null;
  return null;
}
function r22FullBundledPages(sourceId,pages=[]){
  const unique=uniq((pages||[]).map(Number).filter(Number.isFinite)).sort((a,b)=>a-b); if(!unique.length)return '';
  return `<details class="inset-card full-source-pages"><summary><span><b>Full ${esc(sourceId.toUpperCase())} source tables / transcript</b><small>Exact bundled-source parity for the linked pages</small></span><span class="pill">${unique.length}</span></summary><div class="full-source-page-list">${unique.map(p=>{const rec=r22FindSourcePage(sourceId,p),book=rec?.bookPage||rec?.page||p;return `<details class="source-page-card"><summary><b>${esc(sourceId.toUpperCase())} p${esc(book)}</b><span class="muted small">PDF p${esc(p)}</span></summary>${sourcePageEnhancementsHTML(sourceId,p,{open:true})}${sourceTranscriptDetailsHTML(sourceId,p,rec?.text||'')}<button class="link-btn" data-source-ref='${encodeURIComponent(JSON.stringify({source:sourceId,page:book,pdfPage:p}))}'>Open exact bundled PDF page →</button></details>`}).join('')}</div></details>`;
}
function r22MsidcHTML(o){
  const paths=(o.links||[]).map(id=>state.db.resistance.find(r=>r.id===id)).filter(r=>r?.managementGuide&&((r.managementGuide.refs||[]).some(x=>x.source==='msidc')||(r.sourceCards||[]).some(x=>x.source==='msidc')||r.managementGuide.source==='msidc'));
  if(!paths.length&&!o.msidcSourcePages?.length)return '';
  const pages=uniq([
    ...paths.flatMap(r=>(r.managementGuide?.refs||[]).filter(x=>x.source==='msidc').map(x=>x.pdfPage||x.page)),
    ...(o.refs||[]).filter(x=>x.source==='msidc').map(x=>x.pdfPage||x.page)
  ].map(Number).filter(Number.isFinite));
  return `<details class="card organism-source-detail mdr-detail" open><summary><span><b>MSIDC MDR Gram-Negatives 2024</b><small>All linked preferred / less-preferred site and phenotype rows</small></span>${sourcePill('msidc')}</summary><div class="organism-source-body">${paths.map((r,pi)=>{const g=r.managementGuide||{},msRows=(g.siteRows||[]).filter(row=>row.ref?.source==='msidc'||(g.source==='msidc'&&row.ref?.source!=='hpusm')),msCards=(r.sourceCards||[]).filter(sc=>sc.source==='msidc');return `<details class="organism-diagnosis" ${paths.length===1||pi===0?'open':''}><summary><span><b>${esc(r.name)}</b><small>${msRows.length} site/severity row${msRows.length===1?'':'s'}${msCards.length?` · ${msCards.length} source card${msCards.length===1?'':'s'}`:''}</small></span><span class="chev">＋</span></summary>${(g.definition||[]).length?`<div class="source-field"><b>Definition / interpretation</b>${r22List(g.definition)}</div>`:''}${msRows.map(row=>`<div class="organism-source-regimen"><div class="source-field"><b>Infection site / severity</b><span>${esc(row.site)}</span></div><div class="source-field"><b>Preferred</b><span>${esc(row.preferred||'Not separately stated')}</span></div><div class="source-field"><b>Less preferred / additional source-directed option</b><span>${esc(row.alternative||'None separately listed')}</span></div>${row.ref?`<div class="tag-row">${pathwayRefButtons([row.ref])}</div>`:''}</div>`).join('')}${msCards.map(sc=>`<div class="organism-source-regimen"><div class="source-field"><b>MSIDC source detail</b><span>${esc(sc.title||'')}</span></div><div class="source-field"><b>Preferred / less-preferred context</b><span>${esc(sc.text||'')}</span>${(sc.bullets||[]).length?r22List(sc.bullets):''}</div><div class="tag-row">${pathwayRefButtons([{source:'msidc',page:sc.page,pdfPage:sc.pdfPage}])}</div></div>`).join('')}${(g.dosing||[]).length?`<details class="inset-card"><summary><b>Dosing / administration detail</b></summary>${r22List(g.dosing)}</details>`:''}${(g.pearls||[]).length?`<details class="inset-card"><summary><b>Clinical pearls / cautions</b></summary>${r22List(g.pearls)}</details>`:''}${g.duration?`<div class="source-field"><b>Duration principle</b><span>${esc(g.duration)}</span></div>`:''}${(g.ipc||[]).length?`<details class="inset-card"><summary><b>IPC / colonisation context</b></summary>${r22List(g.ipc)}</details>`:''}<div class="btn-row"><button class="secondary-btn" data-open="resistance" data-id="${esc(r.id)}">Open full ${esc(r.name)} pathway</button></div></details>`}).join('')}${r22FullBundledPages('msidc',pages)}</div></details>`;
}
function r22FallbackNagGuideHTML(o){
  const g=o.antimicrobialGuide;if(!g||g.source!=='nag'||state.db.organismSourceDetail?.nagQuickGuide?.[o.id])return '';
  return `<details class="card organism-source-detail nag-detail"><summary><span><b>NAG-linked organism guidance</b><small>Source-labelled quick guidance where the NAG page is not a Preferred/Alternative/Comment table row</small></span>${sourcePill('nag')}</summary><div class="organism-source-body">${(g.groups||[]).map(gr=>`<div class="organism-source-regimen"><div class="source-field"><b>${esc(gr.label||'Source-listed option')}</b><span>${esc(gr.text||'')}</span></div><div class="tag-row">${drugLinkButtons(gr.drugIds||[])}</div></div>`).join('')}${g.comment?`<div class="source-field"><b>Comment</b><span>${esc(g.comment)}</span></div>`:''}${g.guard?`<div class="notice">${esc(g.guard)}</div>`:''}${g.url?`<div class="btn-row"><a class="secondary-btn" href="${esc(g.url)}" target="_blank" rel="noopener">Check current NAG source ↗</a></div>`:''}</div></details>`;
}
function r22OrganismSourceDetailHTML(o){
  const h=r22HpusmHTML(o),nq=r22NagQuickGuideHTML(o),nf=r22FallbackNagGuideHTML(o),ns=r22NagSyndromeHTML(o),m=r22MsidcHTML(o);
  if(!(h||nq||nf||ns||m))return '';
  return `<div class="section-head organism-source-head"><div><h2>Source-specific antimicrobial detail</h2><p>Full organism and linked syndrome detail; sources remain separate and collapsible</p></div></div>${h}${nq}${nf}${ns}${m}`;
}
renderOrganism=function(id){
  recordRoute({view:'entity',type:'organism',id});const o=state.db.organisms.find(x=>x.id===id);if(!o)return;state.lastView={type:'organism',id};addRecent('organism',id,o.name);state.nav='organisms';navActive();
  $('#main').innerHTML=`${entityTop('organism',o,o.gram)}${organismMicrobiologyHTML(o)}${r22OrganismSourceDetailHTML(o)}<div class="card"><h3>Important phenotypes</h3><div class="tag-row">${(o.phenotypes||[]).map(x=>`<span class="pill">${esc(x)}</span>`).join('')}</div>${(o.notes||[]).map(n=>`<div class="notice">${esc(n)}</div>`).join('')}</div><div class="card"><h3>Linked pathways</h3><div class="list">${(o.links||[]).map(id=>{const r=state.db.resistance.find(x=>x.id===id),c=state.db.conditions.find(x=>x.id===id),x=r||c;if(!x)return '';return `<button class="list-row" data-open="${r?'resistance':'condition'}" data-id="${x.id}"><div class="grow"><strong>${esc(x.name)}</strong><small>${r?'Resistance mechanism / phenotype':'Clinical syndrome'}</small></div><span class="chev">›</span></button>`}).join('')}</div></div>${currentNagLinks(o)}<div class="notice warn"><b>Organism ≠ prescription.</b> Site, resistance mechanism, isolate susceptibility, colonisation vs infection, severity, source control and patient factors determine definitive treatment.</div>${footer()}`;scrollTo(0,0);
};
/* ==================== end v0.20.0-r22 organism source-detail expansion ==================== */

/* ==================== v0.20.0-r23 bidirectional source mapping ==================== */
function r23SourceRows(c,sourceId){return (c.recommendations||[]).filter(r=>r.source===sourceId);}
function r23SourceRefButton(r,extraCls=''){
  const ref={source:r.source,page:r.page,pdfPage:r.pdfPage,url:r.url,pdfUrl:r.pdfUrl};
  return `<button class="pill ${extraCls}" data-source-ref='${encodeURIComponent(JSON.stringify(ref))}'>${refText(ref)} →</button>`;
}
function r23ConditionRegimenRow(c,r,opts={}){
  const preferred=r.preferred||[], alt=r.alternative||[], notes=r.notes||[];
  return `<div class="organism-source-regimen">
    <div class="source-field"><b>${esc(opts.contextLabel||'Clinical context / source stratum')}</b><span>${esc(r.label||c.name)}</span></div>
    ${opts.showDiagnosis?`<div class="source-field"><b>Diagnosis / syndrome</b><span>${esc(c.name)}</span></div>`:''}
    <div class="source-field"><b>${esc(opts.preferredLabel||'Preferred')}</b>${r22List(preferred)}</div>
    <div class="source-field"><b>${esc(opts.alternativeLabel||'Alternative / additional')}</b>${r22List(alt)}</div>
    ${r.duration?`<div class="source-field"><b>Duration / review</b><span>${esc(r.duration)}</span></div>`:''}
    ${notes.length?`<div class="source-field"><b>Comment / source note</b>${r22List(notes)}</div>`:''}
    <div class="tag-row">${r23SourceRefButton(r,opts.refCls||'')}</div>
  </div>`;
}
function r23PagesFromCondition(c,sourceId){
  const pages=[];
  (c.recommendations||[]).filter(r=>r.source===sourceId).forEach(r=>{const p=Number(r.pdfPage||r.page);if(Number.isFinite(p))pages.push(p)});
  (c.refs||[]).filter(r=>r.source===sourceId).forEach(r=>{const p=Number(r.pdfPage||r.page);if(Number.isFinite(p))pages.push(p)});
  if(sourceId==='msic') (c.msicChapterLinks||[]).forEach(x=>(x.pdfPages||[]).forEach(p=>{p=Number(p);if(Number.isFinite(p))pages.push(p)}));
  if(sourceId==='hpusm') (c.hpusmChapterLinks||[]).forEach(x=>(x.pages||[]).forEach(p=>{p=Number(p);if(Number.isFinite(p))pages.push(p)}));
  return uniq(pages).sort((a,b)=>a-b);
}
function r23ConditionHpusmHTML(c){
  const rows=r23SourceRows(c,'hpusm'), links=c.hpusmChapterLinks||[], pages=r23PagesFromCondition(c,'hpusm');
  if(!rows.length&&!links.length)return '';
  return `<details class="card organism-source-detail local-detail disease-source-detail" open><summary><span><b>HPUSM ASP 2025</b><small>Diagnosis · risk/source stratum · recommended antimicrobial therapy · duration</small></span>${sourcePill('hpusm')}</summary><div class="organism-source-body">
    <div class="source-field"><b>Diagnosis</b><span>${esc(c.name)}</span></div>
    ${(c.riskFactors||[]).length?`<div class="source-field"><b>Risk stratification / modifiers</b><div class="tag-row">${c.riskFactors.map(x=>`<span class="pill">${esc(x)}</span>`).join('')}</div></div>`:''}
    ${rows.map(r=>r23ConditionRegimenRow(c,r,{contextLabel:'Risk / source stratum',preferredLabel:'Recommended antimicrobial therapy — preferred',alternativeLabel:'Recommended antimicrobial therapy — alternative',refCls:'local'})).join('')}
    ${links.length?`<details class="inset-card"><summary><span><b>HPUSM chapter context</b><small>${links.length} linked chapter${links.length===1?'':'s'}</small></span><span class="chev">＋</span></summary><div class="list">${links.map(x=>`<button class="list-row" data-tool="hpusm" data-chapter="${esc(x.chapter)}"><div class="grow"><strong>Chapter ${esc(x.chapter)} · ${esc(x.title)}</strong><small>HPUSM p${esc((x.pages||[]).join('–'))}</small></div><span class="chev">›</span></button>`).join('')}</div></details>`:''}
    ${r22FullBundledPages('hpusm',pages)}
  </div></details>`;
}
function r23ConditionNagHTML(c){
  const rows=r23SourceRows(c,'nag'), n=c.nagDetail;
  if(!rows.length&&!n)return '';
  const urls=uniq([...rows.map(r=>r.url).filter(Boolean),n?.url].filter(Boolean));
  return `<details class="card organism-source-detail nag-detail disease-source-detail" ${rows.length?'open':''}><summary><span><b>Malaysia NAG</b><small>Preferred · Alternative · Comment / duration · current source link</small></span>${sourcePill('nag')}</summary><div class="organism-source-body">
    ${rows.map(r=>r23ConditionRegimenRow(c,r,{preferredLabel:'Preferred',alternativeLabel:'Alternative',contextLabel:'Diagnosis / severity / source stratum'})).join('')}
    ${n?.points?.length?`<details class="inset-card"><summary><b>${esc(n.title||'Additional NAG detail')}</b></summary>${r22List(n.points)}</details>`:''}
    ${urls.length?`<div class="btn-row">${urls.map(u=>`<a class="secondary-btn" href="${esc(u)}" target="_blank" rel="noopener">Check current NAG source ↗</a>`).join('')}</div>`:''}
    <div class="notice warn"><b>NAG context guard:</b> Apply the source row to the stated syndrome, age group, severity and microbiology context; do not convert an organism-level option into a universal regimen.</div>
  </div></details>`;
}
function r23ConditionMsicHTML(c){
  const rows=r23SourceRows(c,'msic'), links=c.msicChapterLinks||[], pages=r23PagesFromCondition(c,'msic');
  const direct=[...(c.refs||[]),...(c.microbiologyRefs||[])].filter(r=>r.source==='msic');
  if(!rows.length&&!links.length&&!direct.length)return '';
  const principleNotes=uniq([...(c.why||[]),...(c.practicalPathway?.notes||[])].filter(x=>/MSIC|ICU guide|critical-care|PK\/PD|de-escal/i.test(x)));
  return `<details class="card organism-source-detail icu-detail disease-source-detail" open><summary><span><b>Guide to Antimicrobial Therapy in the Adult ICU · MSIC 2023</b><small>Syndrome / severity-specific ICU recommendation · dose/administration · duration · full source context</small></span>${sourcePill('msic')}</summary><div class="organism-source-body">
    <div class="notice"><b>Adult ICU source.</b> These recommendations retain their ICU syndrome/severity context and remain separate from HPUSM, NAG and MSIDC.</div>
    ${(c.riskFactors||[]).length?`<div class="source-field"><b>ICU severity / risk modifiers represented on this pathway</b><div class="tag-row">${c.riskFactors.map(x=>`<span class="pill">${esc(x)}</span>`).join('')}</div></div>`:''}
    ${rows.map(r=>r23ConditionRegimenRow(c,r,{contextLabel:'ICU syndrome / severity / acquisition context',preferredLabel:'Preferred regimen / dose',alternativeLabel:'Alternative / additional regimen',refCls:'icu'})).join('')}
    ${!rows.length&&principleNotes.length?`<div class="source-field"><b>MSIC ICU principle / linked context</b>${r22List(principleNotes)}</div>`:''}
    ${!rows.length&&direct.length?`<div class="notice warn"><b>No stand-alone MSIC regimen row is stored for this syndrome.</b> The MSIC source is linked here for ICU assessment/stewardship context; use the suspected-source pathway for the actual empirical regimen.</div>`:''}
    ${links.length?`<details class="inset-card" open><summary><span><b>Full MSIC chapter context</b><small>${links.length} linked chapter${links.length===1?'':'s'}</small></span><span class="chev">＋</span></summary><div class="list">${links.map(x=>`<button class="list-row" data-tool="msic" data-section="${esc(x.section)}"><div class="grow"><strong>Chapter ${esc(x.chapter)} · ${esc(x.title)}</strong><small>Book p${esc((x.bookPages||[]).join('–'))} · PDF p${esc((x.pdfPages||[]).join('–'))}</small></div><span class="chev">›</span></button>`).join('')}</div></details>`:''}
    ${r22FullBundledPages('msic',pages)}
  </div></details>`;
}
function r23HasMsidcPath(r){
  const g=r?.managementGuide||{};
  return !!r && (g.source==='msidc'||(g.refs||[]).some(x=>x.source==='msidc')||(r.sourceCards||[]).some(x=>x.source==='msidc'));
}
function r23MsidcPathsForCondition(c){
  if(/^paediatric-|^neonatal-/.test(c.id)) return [];
  const ids=new Set();
  (c.organisms||[]).forEach(oid=>{const o=state.db.organisms.find(x=>x.id===oid);(o?.links||[]).forEach(rid=>{const r=state.db.resistance.find(x=>x.id===rid);if(r23HasMsidcPath(r))ids.add(rid)})});
  return [...ids].map(id=>state.db.resistance.find(x=>x.id===id)).filter(Boolean);
}
function r23MsidcRelevantRows(c,r){
  const rows=(r.managementGuide?.siteRows||[]).filter(row=>row.ref?.source==='msidc'||r.managementGuide?.source==='msidc');
  if(!rows.length)return [];
  const hay=slug(`${c.id} ${c.name} ${c.category}`);
  const urinary=/urinar|uti|pyelo|cystitis|prostat|renal perinephric|pyoneph/.test(hay);
  if(urinary){
    const hits=rows.filter(row=>{const site=slug(row.site);return !/outside urinary/.test(site)&&/cystitis|pyelonephritis|cuti|urinary/.test(site)});
    return hits.length?hits:rows;
  }
  const non=rows.filter(row=>!/uncomplicated cystitis|pyelonephritis|cuti/.test(slug(row.site)) || /outside urinary|true infection|mild|moderate|severe|non cp|mdr/.test(slug(row.site)));
  return non.length?non:rows;
}
function r23ConditionMsidcHTML(c){
  const paths=r23MsidcPathsForCondition(c); if(!paths.length)return '';
  const pageSet=[];
  paths.forEach(r=>{(r.managementGuide?.refs||[]).filter(x=>x.source==='msidc').forEach(x=>{const p=Number(x.pdfPage||x.page);if(Number.isFinite(p))pageSet.push(p)})});
  return `<details class="card organism-source-detail mdr-detail disease-source-detail"><summary><span><b>MSIDC MDR Gram-Negatives 2024</b><small>Targeted phenotype therapy relevant to organisms linked with this syndrome</small></span>${sourcePill('msidc')}</summary><div class="organism-source-body">
    <div class="notice warn"><b>Targeted MDR layer — not the default empirical regimen.</b> Use these rows only when the relevant resistant phenotype is confirmed or is being managed as a specific MDR pathway, together with isolate susceptibility, infection site and severity.</div>
    ${paths.map((r,i)=>{const rows=r23MsidcRelevantRows(c,r),g=r.managementGuide||{};return `<details class="organism-diagnosis" ${i===0?'open':''}><summary><span><b>${esc(r.name)}</b><small>${rows.length} relevant site/severity row${rows.length===1?'':'s'}</small></span><span class="chev">＋</span></summary>${(g.definition||[]).length?`<div class="source-field"><b>Definition / interpretation</b>${r22List(g.definition)}</div>`:''}${rows.map(row=>`<div class="organism-source-regimen"><div class="source-field"><b>Infection site / severity</b><span>${esc(row.site)}</span></div><div class="source-field"><b>Preferred</b><span>${esc(row.preferred||'Not separately stated')}</span></div><div class="source-field"><b>Less preferred / alternative</b><span>${esc(row.alternative||'None separately listed')}</span></div>${row.ref?`<div class="tag-row">${pathwayRefButtons([row.ref])}</div>`:''}</div>`).join('')}${g.duration?`<div class="source-field"><b>Duration principle</b><span>${esc(g.duration)}</span></div>`:''}${(g.pearls||[]).length?`<details class="inset-card"><summary><b>Clinical pearls / cautions</b></summary>${r22List(g.pearls)}</details>`:''}<div class="btn-row"><button class="secondary-btn" data-open="resistance" data-id="${esc(r.id)}">Open full ${esc(r.name)} pathway</button></div></details>`}).join('')}
    ${r22FullBundledPages('msidc',pageSet)}
  </div></details>`;
}
function r23ConditionSourceDetailHTML(c){
  const h=r23ConditionHpusmHTML(c),n=r23ConditionNagHTML(c),i=r23ConditionMsicHTML(c),m=r23ConditionMsidcHTML(c);
  if(!(h||n||i||m))return '';
  const count=[h,n,i,m].filter(Boolean).length;
  return `<div id="sourceSpecificSec" class="section-head organism-source-head"><div><h2>Source-specific antimicrobial detail</h2><p>${count} mapped source layer${count===1?'':'s'} · preserved separately by syndrome / severity / phenotype</p></div></div>${h}${n}${i}${m}`;
}
function r23MsicOrganismHTML(o){
  const rows=state.db.conditions.filter(c=>(c.organisms||[]).includes(o.id) && (r23SourceRows(c,'msic').length||(c.msicChapterLinks||[]).length));
  if(!rows.length)return '';
  return `<details class="card organism-source-detail icu-detail" open><summary><span><b>Guide to Antimicrobial Therapy in the Adult ICU · MSIC 2023</b><small>${rows.length} linked ICU syndrome${rows.length===1?'':'s'} · syndrome/severity-specific treatment</small></span>${sourcePill('msic')}</summary><div class="organism-source-body">
    <div class="notice"><b>Context is mandatory.</b> The Adult ICU guide is mapped here through clinical syndromes explicitly linked to this organism. These rows are not a generic susceptibility list.</div>
    ${rows.map(({id})=>{const c=state.db.conditions.find(x=>x.id===id),recs=r23SourceRows(c,'msic'),pages=r23PagesFromCondition(c,'msic');return `<details class="organism-diagnosis"><summary><span><b>${esc(c.name)}</b><small>${recs.length} ICU regimen row${recs.length===1?'':'s'}${(c.msicChapterLinks||[]).length?` · full chapter linked`:''}</small></span><span class="chev">＋</span></summary><div class="source-field"><b>Clinical syndrome / infection site</b><span>${esc(c.name)}</span></div>${(c.riskFactors||[]).length?`<div class="source-field"><b>ICU context / severity / risk modifiers</b><div class="tag-row">${c.riskFactors.map(x=>`<span class="pill">${esc(x)}</span>`).join('')}</div></div>`:''}${recs.map(r=>r23ConditionRegimenRow(c,r,{contextLabel:'ICU syndrome / severity context',preferredLabel:'Preferred regimen / dose',alternativeLabel:'Alternative / additional regimen',refCls:'icu'})).join('')}${(c.msicChapterLinks||[]).length?`<details class="inset-card"><summary><b>Full MSIC chapter / source pages</b></summary><div class="list">${c.msicChapterLinks.map(x=>`<button class="list-row" data-tool="msic" data-section="${esc(x.section)}"><div class="grow"><strong>Chapter ${esc(x.chapter)} · ${esc(x.title)}</strong><small>Book p${esc((x.bookPages||[]).join('–'))}</small></div><span class="chev">›</span></button>`).join('')}</div>${r22FullBundledPages('msic',pages)}</details>`:''}<div class="btn-row"><button class="secondary-btn" data-open="condition" data-id="${esc(c.id)}">Open full ${esc(c.name)} pathway</button></div></details>`}).join('')}
  </div></details>`;
}
function r23OrganismSourceDetailHTML(o){
  const h=r22HpusmHTML(o),nq=r22NagQuickGuideHTML(o),nf=r22FallbackNagGuideHTML(o),ns=r22NagSyndromeHTML(o),i=r23MsicOrganismHTML(o),m=r22MsidcHTML(o);
  if(!(h||nq||nf||ns||i||m))return '';
  return `<div id="sourceSpecificSec" class="section-head organism-source-head"><div><h2>Source-specific antimicrobial detail</h2><p>HPUSM · NAG · Adult ICU guide · MSIDC mapped by diagnosis, syndrome, severity and phenotype</p></div></div>${h}${nq}${nf}${ns}${i}${m}`;
}
renderCondition=function(id){
  recordRoute({view:'entity',type:'condition',id});const c=state.db.conditions.find(x=>x.id===id);if(!c)return;state.lastView={type:'condition',id};addRecent('condition',id,c.name);state.nav='diseases';navActive();
  const sourceLayers=[r23ConditionHpusmHTML(c),r23ConditionNagHTML(c),r23ConditionMsicHTML(c),r23ConditionMsidcHTML(c)].filter(Boolean).length;
  $('#main').innerHTML=`${entityTop('condition',c,c.category+(c.icu?' · ICU':''))}${modeLens()}<div class="card"><p style="font-size:13px;color:var(--ink)">${esc(c.summary)}</p>${c.riskFactors?.length?`<h3 style="margin-top:12px">Key risk modifiers</h3><div class="tag-row">${c.riskFactors.map(x=>`<span class="pill">${esc(x)}</span>`).join('')}</div>`:''}</div>${conditionMicrobiologyHTML(c)}${hpusmSepsisBundleHTML(c)}${conditionDecisionAid(c)}${sourceLayers>1?`<div class="notice warn"><b>Guideline comparison:</b> ${sourceLayers} source layers apply to this page. They are intentionally kept separate rather than merged into a synthetic regimen.</div>`:''}${practicalPathwayHTML(c)}${beforeBox(c.before||[])}${r23ConditionSourceDetailHTML(c)}${conditionReconciliationHTML(c)}${c.why?.length?`<details class="card teaching-details"><summary><b>Why this approach?</b> <span class="muted small">Teaching layer</span></summary><ul style="margin-top:10px">${c.why.map(x=>`<li class="small">${esc(x)}</li>`).join('')}</ul></details>`:''}${currentNagLinks(c)}<div class="card"><h3>Source provenance</h3><div class="tag-row">${(c.refs||[]).map(r=>`<button class="pill" data-source-ref='${encodeURIComponent(JSON.stringify(r))}'>${refText(r)}</button>`).join('')}</div></div>${footer()}`;bindConditionDecisionAid(c);scrollTo(0,0);
};
renderOrganism=function(id){
  recordRoute({view:'entity',type:'organism',id});const o=state.db.organisms.find(x=>x.id===id);if(!o)return;state.lastView={type:'organism',id};addRecent('organism',id,o.name);state.nav='organisms';navActive();
  $('#main').innerHTML=`${entityTop('organism',o,o.gram)}${organismMicrobiologyHTML(o)}${r23OrganismSourceDetailHTML(o)}<div class="card"><h3>Important phenotypes</h3><div class="tag-row">${(o.phenotypes||[]).map(x=>`<span class="pill">${esc(x)}</span>`).join('')}</div>${(o.notes||[]).map(n=>`<div class="notice">${esc(n)}</div>`).join('')}</div><div class="card"><h3>Linked pathways</h3><div class="list">${(o.links||[]).map(id=>{const r=state.db.resistance.find(x=>x.id===id),c=state.db.conditions.find(x=>x.id===id),x=r||c;if(!x)return '';return `<button class="list-row" data-open="${r?'resistance':'condition'}" data-id="${x.id}"><div class="grow"><strong>${esc(x.name)}</strong><small>${r?'Resistance mechanism / phenotype':'Clinical syndrome'}</small></div><span class="chev">›</span></button>`}).join('')}</div></div>${currentNagLinks(o)}<div class="notice warn"><b>Organism ≠ prescription.</b> Site, resistance mechanism, isolate susceptibility, colonisation vs infection, severity, source control and patient factors determine definitive treatment.</div>${footer()}`;scrollTo(0,0);
};
/* ==================== end v0.20.0-r23 bidirectional source mapping ==================== */
