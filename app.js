...
function localDataContextHTML(c){
  if(!c.localDataRelevant) return '';
  const x=state.db.institutionalContext||{}, a=x.antibiogram||{};
  const status=a.status==='NOT_CONFIGURED'?'NOT CONFIGURED':(a.status||'REVIEW');
  const tone=a.status==='NOT_CONFIGURED'?'warn':'good';
  return `<div class="card"><div class="source-meta"><h3 style="margin-right:auto">Institutional empirical context</h3><span class="pill local">${esc(x.policyName||'Local policy')}</span><span class="pill">ANTIBIOGRAM ${esc(status)}</span></div>
    <p class="small">${esc(a.reason||'')}</p>
    <div class="notice ${tone}"><b>How AbxHub handles this:</b> ${esc(a.behavior||'Local susceptibility data are kept separate from guideline recommendations.')}</div>
    <ul class="small">${(x.rules||[]).map(v=>`<li>${esc(v)}</li>`).join('')}</ul>
    ${a.nationalLimitationUrl?`<a class="secondary-btn" href="${esc(a.nationalLimitationUrl)}" target="_blank" rel="noopener">NAG: limitations of cumulative antibiograms ↗</a>`:''}
  </div>`;
}
...
async function init(){
  ...
  try{
    ...
    if('serviceWorker' in navigator) navigator.serviceWorker.register('./sw.js?v=0.18.0',{updateViaCache:'none'}).then(reg=>{reg.addEventListener('updatefound',()=>{const w=reg.installing;if(!w)return;w.addEventListener('statechange',()=>{if(w.state==='installed'&&navigator.serviceWorker.controller)toast('App update ready — reopen to refresh')})})}).catch(()=>{});
  }catch(e){
