...
function organismGuideHTML(o){
  ...
  let guide='';
  if(g){
    guide=`<div class="card organism-guide"><div class="source-meta"><h3 style="margin-right:auto">Antimicrobial quick guide</h3>${sourcePill(g.source||'nag')}</div><div class="notice good"><b>Guideline-linked choices, not an isolate susceptibility report.</b> Use this as rapid reference only; definitive therapy still requires site, phenotype/MIC and patient context.</div>${(g.groups||[]).map(gr=>`<div class="guide-group"><h4>${esc(gr.label)}</h4><p class="small">${esc(gr.text||'')}</p><div class="tag-row">${(gr.drugIds||[]).map(id=>{const d=state.db.drugs.find(x=>x.id===id);return d?`<button class="pill good-pill" data-open="drug" data-id="${id}">${esc(d.name)} ›</button>`:''}).join('')}</div></div>`).join('')}${g.comment?`<div class="notice warn">${esc(g.comment)}</div>`:''}${g.guard?`<div class="notice">${esc(g.guard)}</div>`:''}${g.url?`<a class="secondary-btn" href="${esc(g.url)}" target="_blank" rel="noopener">Check current NAG organism guide ↗</a>`:''}</div>`;
  }
  const linked=(o.links||[]).map(id=>state.db.conditions.find(x=>x.id===id)||state.db.resistance.find(x=>x.id===id)).filter(Boolean);
  const allText=linked.map(x=>x.recommendations?x.recommendations.flatMap(r=>[...(r.preferred||[]),...(r.alternative||[]),...(r.notes||[])]).join(' '):(x.sourceCards||[]).flatMap(sc=>[sc.text,...(sc.bullets||[])]).join(' ')).join(' ');
  const ids=uniq(drugIdsInGuidanceText(allText)).slice(0,20);
  const pathway=`<div class="card"><h3>Antimicrobials appearing in linked AbxHub pathways</h3><p class="small muted">These links are derived from this organism's associated syndrome/resistance guidance. They show where an antimicrobial is discussed—not that every isolate is susceptible.</p><div class="tag-row">${ids.map(id=>{const d=state.db.drugs.find(x=>x.id===id);return d?`<button class="pill" data-open="drug" data-id="${id}">${esc(d.name)} ›</button>`:''}).join('')||'<span class="muted small">No drug-specific linked regimen is structured for this organism; use its syndrome/pathway and isolate susceptibility.</span>'}</div></div>`;
...
function renderCondition(id){
  ...
  $('#main').innerHTML=`${entityTop('condition',c,c.category+(c.icu?' · ICU':''))}${modeLens()}<div class="card"><p style="font-size:13px;color:var(--ink)">${esc(c.summary)}</p>${c.riskFactors?.length?`<h3 style="margin-top:12px">Key risk modifiers</h3><div class="tag-row">${c.riskFactors.map(x=>`<span class="pill">${esc(x)}</span>`).join('')}</div>`:''}</div>${conditionMicrobiologyHTML(c)}${conditionDecisionAid(c)}${sourceCount>1?`<div class="notice warn"><b>Guideline comparison:</b> ${sourceCount} source sets are shown below. They are intentionally not merged into one synthetic regimen.</div>`:''}${beforeBox(c.before||[])}${recommendationsHTML(c)}${nagDetailHTML(c)}${conditionReconciliationHTML(c)}${c.why?.length?`<details class="card teaching-details"><summary><b>Why this approach?</b> <span class="muted small">Teaching layer</span></summary><ul style="margin-top:10px">${c.why.map(x=>`<li class="small">${esc(x)}</li>`).join('')}</ul></details>`:''}${currentNagLinks(c)}<div class="card"><h3>Source provenance</h3><div class="tag-row">${(c.refs||[]).map(r=>`<button class="pill" data-source-ref='${encodeURIComponent(JSON.stringify(r))}'>${refText(r)}</button>`).join('')}</div></div>${footer()}`;bindConditionDecisionAid(c);scrollTo(0,0);
}
function renderOrganism(id){
