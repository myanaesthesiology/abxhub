...
async function init(){
  ...
  try{
    ...
    if('serviceWorker' in navigator) navigator.serviceWorker.register('./sw.js?v=0.16.0',{updateViaCache:'none'}).then(reg=>{reg.addEventListener('updatefound',()=>{const w=reg.installing;if(!w)return;w.addEventListener('statechange',()=>{if(w.state==='installed'&&navigator.serviceWorker.controller)toast('App update ready — reopen to refresh')})})}).catch(()=>{});
  }catch(e){
