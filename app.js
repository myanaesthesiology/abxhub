...
function allSearchItems(){
  const items=[];
  state.db.drugs.forEach(x=>{const p=x.clinicalProfile||{};const bb=p.blueBook?.entries||[];const prep=p.preparation||{}, fluid=p.fluidRestrictedPreparation||{};const searchable=[x.class,x.quickDose,p.mechanism,...(p.adverseEffects||[]),...(p.precautions||[]),...(p.contraindications||[]),...(p.interactions||[]),...bb.flatMap(e=>[e.indications,e.dose,e.adverseReactions,e.contraindications,e.interactions,e.precautions,e.category,e.restriction]),p.paediatric?.dose,...((p.paediatric?.nagCurrent||[]).flatMap(e=>[e.label,e.dose,e.context])),p.sourceReconciliation?.frankShann?.status,p.sourceReconciliation?.frankShann?.note,p.sourceReconciliation?.blueBookFukkm?.snapshot?.status,p.sourceReconciliation?.blueBookFukkm?.snapshot?.note,p.sourceReconciliation?.blueBookFukkm?.current?.status,p.sourceReconciliation?.blueBookFukkm?.current?.note,prep.reconstitution,prep.dilution,prep.diluent,prep.administration,prep.stability,prep.compatibility,prep.comments,fluid.reconstitution,fluid.dilution,fluid.diluent,fluid.administration,fluid.stability,fluid.comments].filter(Boolean).join(' ');items.push({type:'drug',id:x.id,name:x.name,aliases:x.aliases||[],sub:x.class,searchable});});
  state.db.conditions.forEach(x=>{const rec=(x.recommendations||[]).flatMap(r=>[r.label,...(r.preferred||[]),...(r.alternative||[]),r.duration,...(r.notes||[])]);const pp=x.practicalPathway||{};const ptxt=[...(pp.assessment||[]),...(pp.diagnostics||[]),...(pp.regimens||[]).flatMap(r=>[r.label,r.duration]),...(pp.review||[]),...(pp.notes||[])];items.push({type:'condition',id:x.id,name:x.name,aliases:x.aliases||[],sub:x.category,searchable:[x.summary,x.diseaseMechanism,...(x.diagnosticMicrobiology||[]),...rec,...ptxt].filter(Boolean).join(' ')});});
...
function msidcSectionCard(sec,kind,index){
  const links=(sec.pathwayIds||[]).map(id=>{const r=state.db.resistance.find(x=>x.id===id);return r?`<button class="list-row" data-open="resistance" data-id="${esc(id)}"><div class="grow"><strong>${esc(r.name)}</strong><small>Structured AbxHub MDR pathway linked to this MSIDC section</small></div><span class="chev">›</span></button>`:''}).join('');
  return `<div class="source-meta"><span class="pill mdr">${kind==='section'?(sec.number==='P'?'Principles':'Section '+esc(sec.number)):'Appendix '+esc(sec.number)}</span><span class="pill">Book p${esc((sec.bookPages||[]).join('–'))}</span><span class="pill">PDF p${esc((sec.pdfPages||[]).join('–'))}</span></div><h3>${esc(sec.title||'MSIDC section')}</h3>${links?`<div class="list" style="margin:10px 0">${links}</div>`:''}<div class="notice"><b>Source-text parity layer:</b> The transcript preserves the bundled 2024 consensus wording/table text for offline search. For complex table row/column geometry, verify the exact PDF page.</div>${(sec.pages||[]).map(p=>`<details class="inset-card msidc-source-page"><summary><b>MSIDC p${esc(p.bookPage)}</b><span class="muted small">PDF p${esc(p.pdfPage)}</span></summary><pre class="source-transcript">${esc(p.text||'')}</pre><button class="link-btn" data-source-ref='${encodeURIComponent(JSON.stringify({source:'msidc',page:p.bookPage,pdfPage:p.pdfPage}))}'>Open exact bundled PDF page →</button></details>`).join('')}`;
}
...
function hpusmSourcePagesHTML(pages=[]){
  ...
  return `<div class="section-head"><div><h3>Exact HPUSM source pages</h3><p>Searchable transcript + direct bundled-PDF page</p></div></div><div>${wanted.map(p=>`<details class="inset-card hpusm-source-page"><summary><b>HPUSM p${esc(p.page)}</b><span class="muted small">${esc(p.section||'Source page')}</span></summary><pre class="source-transcript">${esc(p.text||'')}</pre><button class="link-btn" data-source-ref='${encodeURIComponent(JSON.stringify({source:'hpusm',page:p.page,pdfPage:p.pdfPage}))}'>Open exact bundled PDF page →</button></details>`).join('')}</div>`;
}
...
function msicSectionCard(sec,kind,index){
  ...
  return `<div class="source-meta"><span class="pill icu">${kind==='chapter'?'Chapter '+esc(sec.number):'Appendix '+esc(sec.letter||String.fromCharCode(65+index))}</span><span class="pill">Book p${esc(sec.bookPages||'')}</span><span class="pill">PDF p${esc(sec.pdfPages||'')}</span></div><h3>${esc(sec.title||'MSIC section')}</h3>${links?`<div class="list" style="margin:10px 0">${links}</div>`:''}<div class="notice"><b>Source-text parity layer:</b> The transcript below preserves the bundled source wording and table text for search/offline review. For complex tables or spatial formatting, open the exact PDF page to verify row/column alignment.</div>${(sec.pages||[]).map(p=>`<details class="inset-card msic-source-page"><summary><b>MSIC book p${esc(p.bookPage)}</b><span class="muted small">PDF p${esc(p.pdfPage)}</span></summary><pre class="source-transcript">${esc(p.text||'')}</pre><button class="link-btn" data-source-ref='${encodeURIComponent(JSON.stringify({source:'msic',page:p.bookPage,pdfPage:p.pdfPage}))}'>Open exact bundled PDF page →</button></details>`).join('')}`;
}
...
function toolWellington(){
  ...
  $('#main').innerHTML=`${toolHeader('Wellington ICU Spectrum 2022','Expected-spectrum source chart · not a susceptibility report')}<div class="notice warn"><b>Historical/source-specific spectrum aid.</b> This 2022 Wellington chart does not replace isolate susceptibility, current local antibiograms, NAG/MSIDC/MSIC guidance or ID/microbiology advice. It predates several newer MDR agents and some printed treatment comments are now historical.</div><div class="home-status"><span class="pill">${(g.columns||[]).length}/17 organism columns</span><span class="pill">${(g.rows||[]).length}/30 structured rows</span><span class="pill">${a.footnotes||0}/8 footnotes</span></div><div class="field"><input id="wellFilter" placeholder="Filter drug or organism (e.g. MRSA, Pseudomonas, cefepime)…"></div><div id="wellRows">${wellingtonRows('')}</div><details class="card" open><summary><b>Source notes and footnotes</b></summary>${(g.generalNotes||[]).map(n=>`<div class="notice">${esc(n)}</div>`).join('')}<ol class="small">${(g.footnotes||[]).map(x=>`<li>${esc(x)}</li>`).join('')}</ol></details><div class="btn-row"><button class="primary-btn" data-source-ref='${encodeURIComponent(JSON.stringify({source:'wellington',page:1,pdfPage:1}))}'>Open original A2 chart ↗</button></div>${footer()}`;
