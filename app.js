...
function prepCardHTML(prep,title='Standard preparation / dilution',tone=''){
  ...
  return `<div class="card ${tone}"><div class="source-meta"><h3 style="margin-right:auto">${esc(title)}</h3>${sourcePill(src)}</div>
    ...
    <div class="notice warn"><b>Product-specific:</b> dilution, stability and compatibility can vary by brand/formulation. Verify the current locally stocked product insert and local pharmacy policy before administration.</div>
...
async function init(){
  ...
  try{
    ...
    if('serviceWorker' in navigator) navigator.serviceWorker.register('./sw.js?v=0.14.0',{updateViaCache:'none'}).then(reg=>{reg.addEventListener('updatefound',()=>{const w=reg.installing;if(!w)return;w.addEventListener('statechange',()=>{if(w.state==='installed'&&navigator.serviceWorker.controller)toast('App update ready — reopen to refresh')})})}).catch(()=>{});
  }catch(e){
