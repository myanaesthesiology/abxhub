...
function allSearchItems(){
  const items=[];
  state.db.drugs.forEach(x=>{const p=x.clinicalProfile||{};const bb=p.blueBook?.entries||[];const searchable=[x.class,x.quickDose,p.mechanism,...(p.adverseEffects||[]),...(p.precautions||[]),...(p.contraindications||[]),...(p.interactions||[]),...bb.flatMap(e=>[e.indications,e.dose,e.adverseReactions,e.contraindications,e.interactions,e.precautions,e.category,e.restriction]),p.paediatric?.dose].filter(Boolean).join(' ');items.push({type:'drug',id:x.id,name:x.name,aliases:x.aliases||[],sub:x.class,searchable});});
...
function drugProfileHTML(d){
  ...
  return `<div class="card drug-profile"><h3>Pharmacology & safety</h3>
    ...
    ${paed?`<details><summary><b>Paediatric / critical-care dose cross-check</b> ${sourcePill('shann')}</summary>${paed.unavailable?`<div class="notice">${esc(paed.note||'No direct DrugDoses v5.5 match.')}</div>`:`<div class="prep-row"><b>Frank Shann DrugDoses v5.5</b><span>${esc(paed.dose||'')}</span></div>`}${paed.blueBookDose?`<details><summary><b>Blue Book/FUKKM dose field for comparison</b></summary><p class="small">${esc(paed.blueBookDose)}</p></details>`:''}<div class="notice warn">${esc(paed.note||'Cross-check only.')}</div></details>`:''}
    ${refButtons(p.refs||[])}
  </div>
  ${prep?`<div class="card"><div class="source-meta"><h3 style="margin-right:auto">Preparation / dilution</h3>${sourcePill(prep.ref?.source||'dailymed')}</div>${prep.reconstitution?`<div class="prep-row"><b>Reconstitution</b><span>${esc(prep.reconstitution)}</span></div>`:''}${prep.dilution?`<div class="prep-row"><b>Dilution</b><span>${esc(prep.dilution)}</span></div>`:''}${prep.administration?`<div class="prep-row"><b>Administration</b><span>${esc(prep.administration)}</span></div>`:''}${prep.compatibility?`<div class="prep-row"><b>Compatibility</b><span>${esc(prep.compatibility)}</span></div>`:''}<div class="notice warn"><b>Product-specific:</b> reconstitution, stability and compatibility vary by manufacturer/formulation. Verify the locally stocked product insert and pharmacy policy before administration.</div>${prep.ref?`<button class="link-btn" data-source-ref='${encodeURIComponent(JSON.stringify(prep.ref))}'>Open preparation source ↗</button>`:''}</div>`:(p.preparationIndex?`<div class="card"><div class="source-meta"><h3 style="margin-right:auto">Preparation / dilution</h3>${sourcePill('moh-dilution')}</div><p class="small">${esc(p.preparationIndex.status)}</p><a class="secondary-btn" href="${esc(p.preparationIndex.url)}" target="_blank" rel="noopener">Open official Sabah dilution protocol ↗</a></div>`:'')}`;
}
...
function toolSources(){
  const ids=['hpusm','nag','msic','msidc','bluebook','myformulary','moh-dilution','shann','wellington','ncbi-mm','dailymed'];
...
async function init(){
  ...
  try{
    ...
    if('serviceWorker' in navigator) navigator.serviceWorker.register('./sw.js?v=0.13.0',{updateViaCache:'none'}).then(reg=>{reg.addEventListener('updatefound',()=>{const w=reg.installing;if(!w)return;w.addEventListener('statechange',()=>{if(w.state==='installed'&&navigator.serviceWorker.controller)toast('App update ready — reopen to refresh')})})}).catch(()=>{});
  }catch(e){
