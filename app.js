...
function allSearchItems(){
  ...
  [
    ...
    {id:'rounds',name:'AMS round',aliases:['antimicrobial stewardship','48 hour review','72 hour review'],sub:'Clinical tool · stewardship',searchable:'AMS antimicrobial stewardship de-escalation cultures source control duration review'},
...
function toolSpectrum(){
  $('#main').innerHTML=`${toolHeader('Spectrum explorer','Expected activity reference with explicit symbol legend')}<div class="notice warn"><b>Spectrum ≠ susceptibility.</b> Isolate-specific susceptibility and current microbiology override this expected-spectrum display.</div><div class="card"><div class="field"><label>Antimicrobial</label><select id="specDrug">${state.db.drugs.map(d=>`<option value="${d.id}">${esc(d.name)}</option>`).join('')}</select></div><div id="specOut"></div></div><div class="card spectrum-legend"><h3>Legend</h3><div class="legend-grid"><span><b>●●●</b> stronger expected activity</span><span><b>●●○</b> intermediate expected activity</span><span><b>●○○</b> limited expected activity</span><span><b>○○○</b> little/no expected activity in this reference layer</span><span><b>✓</b> expected activity</span><span><b>◐</b> partial / conditional</span><span><b>✕</b> not expected</span></div></div>${footer()}`;const draw=()=>{const d=state.db.drugs.find(x=>x.id===$('#specDrug').value),s=d.spectrum;$('#specOut').innerHTML=`<div class="metric-grid">${spectrumMetric('Gram +',s.gramPositive)}${spectrumMetric('Gram −',s.gramNegative)}${spectrumMetric('Pseudomonas',s.pseudomonas)}${spectrumMetric('Anaerobes',s.anaerobes)}${spectrumMetric('MRSA',s.mrsa)}${spectrumMetric('Atypicals',s.atypicals)}</div><div class="btn-row"><button class="secondary-btn" data-open="drug" data-id="${d.id}">Open ${esc(d.name)}</button></div>`};$('#specDrug').addEventListener('change',draw);draw();
}
function toolCompare(){
  const opts=state.db.drugs.map(d=>`<option value="${d.id}">${esc(d.name)}</option>`).join('');
