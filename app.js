...
function toolTDM(){
  ...
  const draw=()=>{const v=$('#tdmDrug').value;$('#tdmBody').innerHTML=v==='vancomycin'?`
    ...
    <div class="notice warn"><b>No unvalidated AUC calculator:</b> AbxHub deliberately does not estimate AUC from a single trough. Use your validated Bayesian platform or institutional two-level PK method.</div>
...
async function init(){
  ...
  try{
    ...
    if('serviceWorker' in navigator) navigator.serviceWorker.register('./sw.js?v=0.15.0',{updateViaCache:'none'}).then(reg=>{reg.addEventListener('updatefound',()=>{const w=reg.installing;if(!w)return;w.addEventListener('statechange',()=>{if(w.state==='installed'&&navigator.serviceWorker.controller)toast('App update ready — reopen to refresh')})})}).catch(()=>{});
  }catch(e){
