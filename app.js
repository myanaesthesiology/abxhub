...
function openSource(id, ref={}){
  ...
  openModal(s.name, `
    <div class="tag-row">${sourcePill(id)}${s.year?`<span class="pill">${esc(s.year)}</span>`:''}${pdf?'<span class="pill good-pill">Bundled PDF</span>':''}</div>
    ...
    <div class="btn-row">
      ${pdfHref?`<a class="primary-btn" href="${esc(pdfHref)}" target="_blank" rel="noopener">Open bundled PDF${pdfPage?` · page ${esc(pdfPage)}`:''} ↗</a>`:''}
...
function pregnancyBox(d){
  ...
  const ref={source:d.pregnancy.source,page:d.pregnancy.page,pdfPage:d.pregnancy.pdfPage,url:d.pregnancy.url,pdfUrl:d.pregnancy.pdfUrl};
...
function drugProfileHTML(d){
  ...
  return `<div class="card drug-profile"><h3>Pharmacology & safety</h3>
    ...
    ${p.adverseEffects?.length?`<details><summary><b>Important adverse effects</b></summary><ul>${p.adverseEffects.map(x=>`<li class="small">${esc(x)}</li>`).join('')}</ul></details>`:''}
    ${p.precautions?.length?`<details><summary><b>Precautions / monitoring</b></summary><ul>${p.precautions.map(x=>`<li class="small">${esc(x)}</li>`).join('')}</ul></details>`:''}
    ${p.contraindications?.length?`<details><summary><b>Contraindications</b></summary><ul>${p.contraindications.map(x=>`<li class="small">${esc(x)}</li>`).join('')}</ul></details>`:''}
    ${p.interactions?.length?`<details><summary><b>Interactions</b></summary><ul>${p.interactions.map(x=>`<li class="small">${esc(x)}</li>`).join('')}</ul></details>`:''}
    ${formulary?`<details><summary><b>MOH formulary / restriction</b></summary>
      ${formulary.category?`<div class="prep-row"><b>Prescriber category</b><span>${esc(formulary.category)}</span></div>`:''}
      ${formulary.indications?`<div class="prep-row"><b>Listed indication(s)</b><span>${esc(formulary.indications)}</span></div>`:''}
      ${formulary.restriction?`<div class="prep-row"><b>Restriction</b><span>${esc(formulary.restriction)}</span></div>`:''}
      ${formulary.dose?`<div class="prep-row"><b>Formulary dose</b><span>${esc(formulary.dose)}</span></div>`:''}
      <p class="small muted">${esc(formulary.status||'Check current MyFormulary/FUKKM.')}</p>
      <div class="btn-row"><a class="secondary-btn" href="${esc(formulary.url||'https://pharmacy.moh.gov.my/en/apps/fukkm')}" target="_blank" rel="noopener">Open MOH FUKKM ↗</a>${formulary.myformularyUrl?`<a class="secondary-btn" href="${esc(formulary.myformularyUrl)}" target="_blank" rel="noopener">Check MyFormulary ↗</a>`:''}</div></details>`:''}
    ${p.paediatric?`<details><summary><b>Paediatric dose cross-check</b></summary><p class="small">${esc(p.paediatric.dose||'')}</p>${p.paediatric.note?`<div class="notice">${esc(p.paediatric.note)}</div>`:''}<div class="btn-row"><a class="secondary-btn" href="${esc(p.paediatric.url||'https://pharmacy.moh.gov.my/en/apps/fukkm')}" target="_blank" rel="noopener">MOH FUKKM ↗</a>${p.paediatric.crossCheckUrl?`<a class="secondary-btn" href="${esc(p.paediatric.crossCheckUrl)}" target="_blank" rel="noopener">Frank Shann source ↗</a>`:''}</div></details>`:''}
    ${refButtons(p.refs||[])}
  </div>
  ${prep?`<div class="card"><div class="source-meta"><h3 style="margin-right:auto">Preparation / dilution</h3>${sourcePill(prep.ref?.source||'dailymed')}</div>
    ...
    ${prep.compatibility?`<div class="prep-row"><b>Compatibility</b><span>${esc(prep.compatibility)}</span></div>`:''}
    <div class="notice warn"><b>Product-specific:</b> reconstitution, stability and compatibility vary by manufacturer/formulation. Verify the locally stocked product insert and pharmacy policy before administration.</div>
    ${prep.ref?`<button class="link-btn" data-source-ref='${encodeURIComponent(JSON.stringify(prep.ref))}'>Open preparation source ↗</button>`:''}
  </div>`:''}`;
}
...
function conditionMicrobiologyHTML(c){
  if(!c.diseaseMechanism && !c.organisms?.length) return '';
  return `<div class="card"><h3>Microbiology / disease mechanism</h3>${c.diseaseMechanism?`<p class="small">${esc(c.diseaseMechanism)}</p>`:''}
    ${c.organisms?.length?`<div class="list">${c.organisms.map(id=>{const o=state.db.organisms.find(x=>x.id===id);return o?`<button class="list-row" data-open="organism" data-id="${o.id}"><div class="grow"><strong>${esc(o.name)}</strong><small>${esc(o.gram||'')}</small></div><span class="chev">›</span></button>`:''}).join('')}</div>`:''}
...
function renalResultHTML(d,rr,crcl,rrt){
  ...
  return `<div class="card"><div class="source-meta"><h3 style="margin-right:auto">Patient-specific renal reference</h3>${sourcePill(rr.source)}</div><div class="result-box"><strong>${esc(d.name)}</strong><dl>${crcl!=null?`<dt>CrCl</dt><dd>${Number(crcl).toFixed(1)} mL/min</dd>`:''}${rrt?`<dt>RRT</dt><dd>${esc(rrt)}</dd>`:''}<dt>Regimen</dt><dd>${esc(text||rr.normal)}</dd></dl></div>${rr.high?`<div class="notice"><b>High-dose context:</b> ${esc(rr.high)}</div>`:''}${rr.note?`<div class="notice warn">${esc(rr.note)}</div>`:''}<button class="link-btn" data-source-ref='${encodeURIComponent(JSON.stringify({source:rr.source,page:rr.page,pdfPage:rr.pdfPage}))}'>${refText({source:rr.source,page:rr.page,pdfPage:rr.pdfPage})} →</button><div class="notice"><b>NAG renal cross-check:</b> NAG Appendix 2 is also part of the national source set. Its adult dose table is embedded as a source PDF, so this build keeps the verified MSIC numeric rule above and links the official NAG table rather than guessing a transcription.</div><a class="secondary-btn" href="${esc(nagRenal)}" target="_blank" rel="noopener">Check latest NAG renal table ↗</a></div>`;
}
...
function toolTDM(){
  ...
  $('#main').innerHTML=`${toolHeader('Therapeutic drug monitoring','NAG + MSIC source-specific monitoring; not a Bayesian dosing engine')}<div class="card"><div class="field"><label>Antimicrobial</label><select id="tdmDrug"><option value="vancomycin">Vancomycin</option><option value="aminoglycoside">Gentamicin / Amikacin</option></select></div><div id="tdmBody"></div></div>${footer()}`;
  const draw=()=>{const v=$('#tdmDrug').value;$('#tdmBody').innerHTML=v==='vancomycin'?`
    <div class="quick-dose"><label>VANCOMYCIN</label><strong>Use source-specific loading/renal dosing, then individualise by TDM.</strong></div>
    <div class="notice good"><b>National NAG:</b> 24-h AUC/MIC is the efficacy exposure metric; TDM is indicated when treatment is expected for &gt;48 h, with more frequent monitoring when PK is unstable.</div>
    <div class="notice good"><b>MSIC ICU:</b> its ICU appendix lists AUC24/MIC 400–600 and provides renal/RRT sampling schedules.</div>
    <ul class="small"><li>Normal renal function: MSIC initial pre-level 30 min before 4th dose and post-level 1 h after completion of 4th dose in its cited approach.</li><li>CrCl 30–50: initial assay 24 h after first maintenance dose.</li><li>CrCl &lt;30: initial assay 24 h after loading dose, then redose by TDM.</li><li>HD/CRRT: use the dedicated RRT schedule and TDM; effluent rate matters in CRRT.</li></ul>
    ...
    <div class="notice good"><b>NAG EID/SDD:</b> source-listed exclusions include pregnancy, ascites, burns &gt;20%, endocarditis, CrCl &lt;30 mL/min, dialysis, neutropenia, Gram-positive synergy indications, haemodynamic instability, hearing/vestibular disease and mycobacterial infection.</div>
    <ul class="small"><li>MSIC single-daily dosing: sampling guidance includes post-dose concentrations after the 2nd dose (2 h and 6 h).</li><li>Conventional dosing: trough 30 min before next dose; peak 30 min after completion of a 30-min infusion.</li><li>Repeat more frequently with changing renal function or concurrent nephrotoxic drugs.</li></ul><div class="notice warn">Use your institution's validated aminoglycoside nomogram/TDM workflow for dose adjustment.</div><a class="secondary-btn" href="${esc(pkUrl)}" target="_blank" rel="noopener">Check latest NAG aminoglycoside guide ↗</a>`;
...
function toolSources(){
  const ids=['hpusm','nag','msic','msidc','myformulary','moh-dilution','dailymed','ncbi-mm','wellington','shann'];
  $('#main').innerHTML=`${toolHeader('Sources & app','Source provenance is part of every clinical record')}<div class="card"><h3>Default display hierarchy</h3><ol class="small"><li>HPUSM ASP 2025 — local guidance where applicable</li><li>Malaysia NAG — structured national snapshot + official live freshness link</li><li>MSIC Adult ICU 2023 — ICU dosing, PK/PD, RRT, TDM and special populations</li><li>MSIDC MDR Gram-Negative 2024 — dedicated MDR mechanism/phenotype guidance</li><li>Wellington ICU spectrum chart — rapid expected-spectrum reference only</li></ol><div class="notice">Dedicated guidance may outrank the general hierarchy for its own domain, especially MSIDC for MDR Gram-negative infections. The app keeps source-specific recommendations separate rather than silently reconciling them.</div></div><div class="list">${ids.map(id=>{const s=source(id);return `<button class="list-row" data-source="${esc(id)}"><div class="grow"><strong>${esc(s.name)}</strong><small>${esc(s.note||'')}</small></div><span class="chev">›</span></button>`}).join('')}</div><div class="section-head"><div><h2>Bundled PDF reference library</h2><p>Every uploaded PDF source is included in this GitHub build; PDFs cache on demand after first opening.</p></div></div><div class="list">${(state.db.bundledReferences||[]).map(r=>`<a class="list-row" href="${esc(r.file)}" target="_blank" rel="noopener" style="text-decoration:none"><div class="grow"><strong>${esc(r.label)}</strong><small>Bundled source PDF</small></div><span class="chev">PDF ↗</span></a>`).join('')}</div><div class="notice warn"><b>Expanding clinical database.</b> This version substantially expands high-yield adult/ICU drugs, syndromes, NAG pathways, renal/RRT data, MDR algorithms, organism microbiology and drug pharmacology/preparation fields. It still requires clinical validation and further chapter-by-chapter extraction before institutional release as a comprehensive reference.</div>${footer()}`;
}
...
async function init(){
  ...
  try{
    ...
    $('#main').innerHTML=`<div class="empty"><b>Unable to initialise AbxHub</b>${esc(e.message)}<br><br>If this is a GitHub Pages deployment, refresh after the service worker update. If testing locally, serve the folder through HTTPS/localhost rather than opening index.html directly.</div>`;
  }
}
init();
