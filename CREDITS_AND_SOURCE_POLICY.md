...
- **Blue Book formulary APK snapshot** — structured offline formulary/safety fields extracted from the user-supplied APK; raw APK not redistributed. Live MyFormulary/FUKKM remains the freshness check.
- **Frank Shann DrugDoses v5.5 APK** — structured paediatric/critical-care cross-check fields extracted from the user-supplied APK; raw APK not redistributed.
- **MOH Dilution Guideline for Injectable Drugs (Part I – Antimicrobial)** — primary standard reconstitution/dilution/administration source for matched injectable antimicrobials.
- **MOH Dilution Guide for Fluid Restricted Critically Ill Adults (Dec 2025)** — separate critical-care minimum-volume overlay; official live source is linked.
- **JKN Sabah Injectable Drug Dilution Protocol, 1st Edition** — retained only as an explicit fallback for selected agents not yet covered by the national standard in AbxHub.
