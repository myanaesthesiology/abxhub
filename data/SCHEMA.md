...
## v0.18 validation-gate objects

### `institutionalContext`
Separates local empirical **policy** from local cumulative **antibiogram** data. A missing antibiogram must remain explicitly `NOT_CONFIGURED`; do not infer susceptibility percentages from national data or expected-spectrum tables.

### `rrtValidation`
Global validation rules for CVVH/CVVHD/CVVHDF interpretation, including modality, actual effluent rate, downtime/running state, residual renal function and TDM requirements.

### `tdm.*.workflowValidation`
Lists required inputs, accepted source methods and institution-specific configuration still required before an automatic dosing workflow can be enabled.

### `paediatricValidation.lineByLineCurrentNAG`
Audit records for high-risk current-NAG paediatric/neonatal dose pathways. These records document reconciliation status; they do not replace the source-specific dose objects on drug/condition records.

### `preparationValidation`
Tracks national MOH preparation coverage separately from local stocked-product, stability and Y-site compatibility sign-off.
