...
if f'sw.js?v={ASSET_REV}' not in app: err('app.js service-worker asset revision mismatch')
for needle in [f'app.js?v={ASSET_REV}',f'styles.css?v={ASSET_REV}',f'manifest.webmanifest?v={ASSET_REV}']:
    if needle not in index: err(f'index missing versioned asset {needle}')
if f"const CACHE = 'abxhub-v{ASSET_REV}'" not in sw: err('service worker cache revision mismatch')
...
if nag.get('lastChecked')!='2026-09-11': warn('NAG lastChecked differs from release audit date')
